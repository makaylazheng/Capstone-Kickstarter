...
video_paths_csv = pd.read_csv(
    join(root, 'code', 'corrected_video_paths.csv')
    )
...
def run_video(video_path):
    ...
    # load models
    learn = load_learner(
        join(root, 'code', '01-analysis_video')
        , 'export.pkl'
        )
    face_cascade = cv2.CascadeClassifier(
        join(root, 'code', '01-analysis_video', 'haarcascade_frontalface_default.xml')
        )
    predictor = dlib.shape_predictor(
        join(root, 'code', '01-analysis_video', 'shape_predictor_68_face_landmarks.dat')
        )
    framecount = 0
    data=[]
    while vidcap.more():
...
for i in range(video_paths_csv.shape[0]):
    ...
    df.to_csv(join(root, 'code', '01-analysis_video', 'video_emotion_csvs', csv_name))

#%% calculate the emotion percentage for each video

csv_root = join(root, 'code', '01-analysis_video', 'video_emotion_csvs')
...
analysis_video_results.to_csv(join(root, 'code', '01-analysis_video', 'analysis_video_results.csv'))
